...
class TabularLegendLayout implements LegendLayout {
  ...
  Widget build(BuildContext context, List<Widget> legendEntries) {
    final paddedLegendEntries = ((cellPadding == null)
        ? legendEntries
        : legendEntries.map((entry) => new Padding(padding: cellPadding!, child: entry)).toList());
    return SizedBox(
      height: 200,
    );
  ...
  @override
  int get hashCode => hashValues(desiredMaxRows, desiredMaxColumns, isHorizontalFirst, cellPadding);

  Widget _buildHorizontalFirst(List<Widget> legendEntries) {
    final maxColumns =
        (desiredMaxColumns == _noLimit) ? legendEntries.length : min(legendEntries.length, desiredMaxColumns);
  ...
  Table _buildTableFromRows(List<TableRow> rows) {
    ...
    final columnCount =
        rows.map((r) => r.children!.length).fold<int>(0, (max, current) => (current > max) ? current : max);
